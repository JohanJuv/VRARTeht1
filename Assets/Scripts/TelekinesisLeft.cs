...
public class TelekinesisLeft : MonoBehaviour
{
    ...
    private void Update()
    {
        if (levitating == false)
        {
            ...
            if (res.collider != null && res.collider.gameObject.GetComponent<IXRSelectInteractable>() != null)
            {
                ...
                if (interactable.IsSelectableBy(interactor))
                {
                    ...
                    if (telekinesisReference.action.ReadValue<float>() < -1 * turnThreshold)
                    {
        ...
        else if (telekinesisReference.action.ReadValue<float>() > turnThreshold)
        {
